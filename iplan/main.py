...
class IPlanApplication(Adw.Application):
    ...
    def __init__(self):
        super().__init__(
            application_id="ir.imansalmani.iplan", flags=Gio.ApplicationFlags.FLAGS_NONE
        )

        check_database()

        self.create_action(
            "quit", callback=lambda *args: self.quit(), shortcuts=["<Ctrl>q"]
        )
        self.create_action("about", callback=self.app_about_cb)
        self.create_action(
            "shortcuts", callback=self.app_shortcuts_cb, shortcuts=["<Ctrl>question"]
        )
        self.create_action(
            "preferences", callback=self.app_preferences_cb, shortcuts=["<Ctrl>comma"]
        )
        self.create_action("search", callback=self.app_search_cb, shortcuts=["<Ctrl>f"])
        self.create_action(
            "modal-close", callback=self.app_modal_close_cb, shortcuts=["Escape"]
        )
