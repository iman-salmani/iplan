...
def read_tasks(
    project_id: int, list_id: int = None, done_tasks: bool = None
) -> Mapping[Task, list]:
...
def read_task(task_id: int) -> Task:
    ...
    return Task.new_from_record(
        cursor.execute(f"SELECT * FROM tasks WHERE id = {task_id}").fetchone()
    )
...
def update_task(task: Task, move_position=False) -> None:
    ...
    if move_position:
        ...
        if old_task._list != task._list:
            ...
            decrease_position_tasks = cursor.execute(
            ...
            increase_position_tasks = cursor.execute(
                f"""SELECT * FROM tasks WHERE
