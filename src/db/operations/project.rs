...
pub fn read_projects(archive: bool) -> Result<Vec<Project>> {
    let filters = if archive {
        "ORDER BY i DESC"
    } else { "WHERE archive = false" };
    let conn = get_connection();
    let mut stmt = conn.prepare(
        &format!("SELECT * FROM projects {filters}"))?;
...
pub fn read_project(project_id: i64) -> Result<Project> {
    ...
    stmt.query_row([project_id], |row| Project::from_row(row))
}
...
pub fn find_projects(text: &str, archive: bool) -> Result<Vec<Project>> {
    ...
    let conn = get_connection();
    let mut stmt = conn.prepare(
        &format!("SELECT * FROM projects WHERE name LIKE ? ESCAPE '\\' {filters}"))?;
...
fn new_index() -> i64 {
    let conn = get_connection();
    let mut stmt = conn.prepare("SELECT i FROM projects ORDER BY i DESC")
        .expect("Failed to find new index");
    let first_row = stmt.query_row([], |row| row.get::<_, i64>(0));
    match first_row {
